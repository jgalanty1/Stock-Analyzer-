...
def fetch_institutional_filings(cik: str, session, limiter,
    ...
    # Look for institutional disclosure forms filed about this company
    # Note: SC 13D/G are filed by the INVESTOR about the COMPANY
    # They appear in the company's filing list on EDGAR
    ...
    for i in range(len(forms)):
        ...
        if form_type in institutional_forms and filing_date >= cutoff:
            filings.append({
                "form_type": form_type,
                "filing_date": filing_date,
                "accession": accessions[i] if i < len(accessions) else "",
